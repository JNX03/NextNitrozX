...
module.exports = (() => {
	const config = {
		"info": {
			...
			"authors": [{
				"name": "JN03",
				"discord_id": "634409440837238784",
				"github_username": "JNDEV03"
			}],
			"version": "1.x",
			"description": "Unlock all screensharing modes, and use cross-server & GIF emotes!",
			"github": "https://github.com/JNDEV03/NextNitrozX",
			"github_raw": "https://github.com/JNDEV03/NextNitrozX/NextNitrozX/main/NextNitrozX.plugin.js"
		},
		"main": "NextNitrozX.plugin.js"
	};
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => {
			...
			return class NextNitrozX extends Plugin {
				defaultSettings = {
					"emojiSize": 64,
					"screenSharing": true,
					"emojiBypass": true,
				...
				saveAndUpdate(){ //Saves and updates settings and runs functions
					...
					if(this.settings.emojiBypass){
						this.emojiBypass()
						BdApi.Patcher.instead("NextNitrozX", permissions, "canUseAnimatedEmojis", () => {
							return true
						});
						BdApi.Patcher.instead("NextNitrozX", permissions, "canUseEmojisEverywhere", () => {
							return true
						});
					}
					...
					if(this.settings.profileV2 == true){
						BdApi.Patcher.instead("NextNitrozX", permissions, "isPremiumAtLeast", () => {
							return true;
						});
					}
					...
					if(this.settings.screenSharing){
						...
						BdApi.Patcher.instead("NextNitrozX", permissions, "canStreamMidQuality", () => {
							return true;
						});
						BdApi.Patcher.instead("NextNitrozX", permissions, "canStreamMedQuality", () => {
							return true;
						});
						BdApi.Patcher.instead("NextNitrozX", permissions, "canStreamHighQuality", () => {
							return true;
						});
					...
					if(!this.settings.emojiBypass && !this.settings.stickerBypass && !this.settings.screenSharing){
						...
						if(this.originalNitroStatus == (null || undefined)){
							BdApi.Patcher.instead("NextNitrozX", permissions, "canUseStickersEverywhere", () => {
								return false;
							});
						}
					}
					...
					if(this.settings.activityBypass){
						BdApi.Patcher.instead("NextNitrozX", permissions, "canUseActivities", () => {
							return true;
						});
						BdApi.Patcher.instead("NextNitrozX", permissions, "canUsePremiumActivities", () => {
							return true;
						});
						this.activities();
					}
				...
				async customVideoSettings() {
					...
					function replace60FPSRequirements(x) {
						if(x.fps != 30 && x.fps != 15 && x.fps != 5) x.fps = BdApi.getData("NextNitrozX","settings").CustomFPS;
					}
					...
					if(BdApi.Webpack.getModule(BdApi.Webpack.Filters.byStrings("updateRemoteWantsFramerate"))){
						...
						//console.log(L);
						if(L){
							BdApi.Patcher.instead("NextNitrozX", L, "updateRemoteWantsFramerate", () => {
							return
						});
						}
						return
						...
						if(R){
							BdApi.Patcher.instead("NextNitrozX", R, "updateRemoteWantsFramerate", () => {
							return
						});
						}
					}
				...
				async emojiBypass(){ //Moved to a function to declutter saveAndUpdate
					...
					if(this.settings.uploadEmotes) {
						BdApi.Patcher.instead("NextNitrozX", DiscordModules.MessageActions, "sendMessage", (_, b, send) => {
					...
					if(this.settings.ghostMode && !this.settings.uploadEmotes) {
						BdApi.Patcher.before("NextNitrozX", DiscordModules.MessageActions, "sendMessage", (_, [, msg]) => {
							var currentChannelId = BdApi.findModuleByProps("getLastChannelFollowingDestination").getChannelId();
						...
						if(!this.settings.ghostMode && !this.settings.uploadEmotes) {
							...
							BdApi.Patcher.before("NextNitrozX", DiscordModules.MessageActions, "editMessage", (_, obj) => {
				...
				updateQuick(){ //Function that runs when the resolution/fps quick menu is changed
					...
					BdApi.getData("NextNitrozX", "settings").CustomResolution = parseInt(document.getElementById("qualityInput").value);
					parseInt(document.getElementById("qualityInputFPS").value);
					BdApi.getData("NextNitrozX", "settings").CustomFPS = parseInt(document.getElementById("qualityInputFPS").value);
					if(parseInt(document.getElementById("qualityInputFPS").value) == 15) BdApi.getData("NextNitrozX", "settings").CustomFPS = 16;
					if(parseInt(document.getElementById("qualityInputFPS").value) == 30) BdApi.getData("NextNitrozX", "settings").CustomFPS = 31;
					if(parseInt(document.getElementById("qualityInputFPS").value) == 5) BdApi.getData("NextNitrozX", "settings").CustomFPS = 6;
					
					const StreamButtons = ZLibrary.WebpackModules.getByIndex(664637);
					if(BdApi.getData("NextNitrozX", "settings").ResolutionEnabled){
						if(BdApi.getData("NextNitrozX", "settings").CustomResolution != 0){
							StreamButtons.LY.RESOLUTION_SOURCE = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							StreamButtons.ND[0].resolution = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							StreamButtons.ND[1].resolution = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							StreamButtons.ND[2].resolution = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							StreamButtons.ND[3].resolution = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							StreamButtons.WC[2].value = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							delete StreamButtons.WC[2].label;
							StreamButtons.WC[2].label = BdApi.getData("NextNitrozX", "settings").CustomResolution.toString();
							StreamButtons.km[3].value = BdApi.getData("NextNitrozX", "settings").CustomResolution;
							delete StreamButtons.km[3].label;
							StreamButtons.km[3].label = BdApi.getData("NextNitrozX", "settings").CustomResolution + "p";
						}
					}
					if(!BdApi.getData("NextNitrozX", "settings").ResolutionEnabled || (BdApi.getData("NextNitrozX", "settings").CustomResolution == 0)){
						StreamButtons.LY.RESOLUTION_SOURCE = 0;
						StreamButtons.ND[0].resolution = 0;
						StreamButtons.ND[1].resolution = 0;
						StreamButtons.ND[2].resolution = 0;
					...
					if(BdApi.getData("NextNitrozX", "settings").CustomFPSEnabled){
						if(this.CustomFPS != 60){
							StreamButtons.ND.forEach(replace60FPSRequirements);
							StreamButtons.af[2].value = BdApi.getData("NextNitrozX", "settings").CustomFPS;
							delete StreamButtons.af[2].label;
							StreamButtons.af[2].label = BdApi.getData("NextNitrozX", "settings").CustomFPS + " FPS";
							StreamButtons.k0[2].value = BdApi.getData("NextNitrozX", "settings").CustomFPS;
							delete StreamButtons.k0[2].label;
							StreamButtons.k0[2].label = BdApi.getData("NextNitrozX", "settings").CustomFPS;
							StreamButtons.ws.FPS_60 = BdApi.getData("NextNitrozX", "settings").CustomFPS;
						}
					}
					if(!(BdApi.getData("NextNitrozX", "settings").CustomFPSEnabled)){
						StreamButtons.ND.forEach(restore60FPSRequirements);
				...
				audioShare(){
					...
					BdApi.Patcher.before("NextNitrozX", shareModule, "setSoundshareSource", (a,b) => {
						if(this.settings.audioSourcePID == 0){
							return
						}
				...
				videoQualityModule(){ //Custom Bitrates
					...
					if(this.settings.CustomBitrateEnabled){
						BdApi.Patcher.before("NextNitrozX", videoOptionFunctions, "updateVideoQuality", (e) => {
							//Minimum Bitrate
					...
					if(this.settings.CameraSettingsEnabled){ //If Camera Patching On
						BdApi.Patcher.after("NextNitrozX", videoOptionFunctions, "updateVideoQuality", (e) => {
				...
				buttonCreate(){ //Creates the FPS and Resolution Swapper
					...
					try{
						...
						console.warn("NextNitrozX: What the fuck happened? During buttonCreate()");
						console.log(err);
					};
				...
				async stickerSending(){
					...
					BdApi.Patcher.instead("NextNitrozX", DiscordModules.MessageActions, "sendStickers", (_,b) => {
						let stickerID = b[1][0];
				...
				activities(){
					...
					BdApi.Patcher.before("NextNitrozX", BdApi.React, "createElement", (_,h) => {
						if(h[1]) if(h[1].className) if(h[1].className == "activityItem-1Z9CTr"){
							...
							if(textField) if(textField.innerText === "Have feedback? Take the survey"){
								BdApi.getData("NextNitrozX", "settings").activityJoiningMode = false;
								...
								let asdf = function(){
									...
									BdApi.getData("NextNitrozX", "settings").activityJoiningMode = true;
					...
					BdApi.Patcher.instead("NextNitrozX", d, "Z", (_,N,f) => {
						if(N !== undefined){
